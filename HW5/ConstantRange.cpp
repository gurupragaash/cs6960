...
void knownBits(ConstantRange data, APInt &ones, APInt &zeros) {
  bool Overflow = true;
	APInt diff = data.getUpper().usub_ov(data.getLower(), Overflow);
  unsigned bits = diff.getActiveBits(); 
	unsigned zerobits;
	zerobits = bits;
	if (bits > 0) {
		zerobits--;
  ...
  if (Overflow) {
		...
		zeros = APInt::getMaxValue(data.getUpper().getBitWidth());
		ones = APInt::getMinValue(data.getUpper().getBitWidth());
		return;
	}
	APInt OneMask = APInt::getAllOnesValue(data.getUpper().getBitWidth()) << bits; 
  //APInt ZeroMask = APInt::getLowBitsSet(data.getUpper().getBitWidth(), zerobits);
  ...
  ones = (data.getUpper() & data.getLower()) & OneMask; 

  //Same logic as above but now | them
  zeros = (data.getUpper() | data.getLower()) | (~OneMask); 
}
...
ConstantRange::binaryAnd(const ConstantRange &Other) const {
  ...
  //If both the intervals are constant, just and them
  if (isSingleElement() && Other.isSingleElement()) {
    return ConstantRange(getLower() & Other.getLower());
  }

	if ((isWrappedSet() != true) && (Other.isWrappedSet() != true)) {
		APInt knownOnesThis, knownZerosThis, knownOnesOther, knownZerosOther;
		APInt highest;
		knownBits(*this, knownOnesThis, knownZerosThis);
		knownBits(Other, knownOnesOther, knownZerosOther); 
		if (getUpper().ugt(Other.getUpper())) {
			highest = (Other.getUpper() | knownZerosOther) & knownZerosThis;
			highest = (highest.ugt(Other.getUpper())) ? (Other.getUpper() & (knownZerosOther | knownZerosThis)) : highest;
			highest = Other.getUpper() & (knownZerosOther | knownZerosThis);
		} else {
			highest = (getUpper() | knownZerosThis) & knownZerosOther;
			highest = (highest.ugt(getUpper())) ? (getUpper() & (knownZerosOther | knownZerosThis)) : highest;
			highest = getUpper() & (knownZerosOther | knownZerosThis);
		}

		return ConstantRange((knownOnesThis & knownOnesOther), highest);
	}
